...
export default function ProjectDetailTabBlock({id = ""}: {id: string}) {
    ...
    async function getBlocks() {
        setLoading(true)
        const b = await (await fetch('/api/projects/blocks/'+ id)).json()
        setBlocks(b)
        setLoading(false)
        initTWE({ Collapse });
    }
    ...
    return(
                                ...
                                <div id="accordionBlock">
                                    {
                                        blocks.map((block:any, index:any) => {
                                            return (
                                                <div className="rounded-t-lg border border-neutral-200 bg-white dark:border-neutral-600 dark:bg-body-dark mb-2" key={index}>
        ...
        {state.isAddingBlock && <ProjectDetailTabBlockNewBlock id={id} refreshBlocks={() => { getBlocks() } } />}
        {state.isAddingLot && <ProjectDetailTabBlockNewLot id={id}  refreshBlocks={() => { getBlocks() } } />}
        {state.isEditingBlock && <ProjectDetailTabBlockUpdateBlock blockID={currentBlock} id={id} refreshBlocks={() => { getBlocks() } }/>}
        {state.isEditingLot && <ProjectDetailTabBlockUpdateLot id={id}  refreshBlocks={() => { getBlocks() } }/>}
    </div>
        </>
    )
}